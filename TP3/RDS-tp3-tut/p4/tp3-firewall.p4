...
parser MyParser(packet_in packet,
    ...
    state parse_ethernet {
        packet.extract(hdr.ethernet); //extract function populates the header
...
control MyIngress(inout headers hdr,
    ...
    */
    ...
    action tcp_fwd(ip4Addr_t nxt_hop, egressSpec_t port){
        if(hdr.tcp.dstPort == 5555 && port == 2 ||
           hdr.tcp.srcPort == 5555){
            ipv4_fwd(nxt_hop, port);
        }
    }
